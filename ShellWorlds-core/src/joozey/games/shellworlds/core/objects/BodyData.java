...
public class BodyData extends GameObject2DData
{
    ...
    private double axisRotationSpeed;
    ...
    public BodyData( BodyType bodyType, float distance, float size, float reach )
    {
        ...
        this.years = 0;
        this.axisRotationSpeed = Math.random() * 0.1f;

        this.mass = .5f;//(float)Math.random() * 0.8f + 0.2f;
        this.density = 0f;//(float)Math.random() * 0.9f + 0.1f;
        this.atmosphere = .5f;//(float)Math.random() * 1f;
        this.diversity = 1f;//(float)Math.random() * 1f;
    ...
    public void addAxisRotation( double amount )
    {
    	this.axisRotation += amount/(Math.PI*2.);
    }
    ...
    public double getAxisRotationSpeed()
    {
    	return this.axisRotationSpeed;
    }
    ...
    public String readDiversityInfo() {
        return "Diversity variable: " + this.diversity;
    }
    ...
    {
        return (ArrayList<BodyObject>) this.reachableBodyList.clone();
    }
