...
public class BodyObject extends GameObject2D
{
    ...
    public BodyObject( BodyData.BodyType type, DefaultSprite image, float distance, float size, float reach )
    {
        bodyView = new BodyView( image );
        bodyData = new BodyData( type, distance, size * bodyView.getWidth(), reach );
        bodyView.setData( bodyData );
    ...
    public void run()
    {
        ...
        bodyData.addAxisRotation( bodyData.getAxisRotationSpeed() * GameData.getSpeed() );
